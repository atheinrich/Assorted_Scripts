...
def new_game():
    ...
    hp_set, defense_set, power_set, exp_set = 100, 100, 100, 0
    fighter_component = Fighter(hp=hp_set, defense=defense_set, power=power_set, exp=exp_set, death_function=player_death)
    ...
    obj = Object(0, 0, images[10], 'dagger', equipment=equipment_component, equipment_list=[slot_set, power_bonus_set])
...
def make_home():
    ...
    level_map = [[ Tile(True, x, y) # Initializes walls at each point in the map.
...
def place_objects(room, home=False, fresh_start=True):
    ...
    if home: # Places items in home
        if fresh_start:
            ...
            monster = Object(x*TILE_WIDTH, y*TILE_HEIGHT, images[3], 'orc', blocks=True, fighter=fighter_component, ai=ai_component, image_num=3, fighter_list=[hp_set, defense_set, power_set, exp_set, 'monster_death'], appended='objects')
            home_objects.append(item)
            home_object_positions.append([x, y])
        ...
        for i in range(num_monsters): # Places monsters
            ...
            if not is_blocked(x, y):
                ...
                if choice == 'orc': # Creates an orc
                    ...
                    fighter_component = Fighter(hp=hp_set, defense=defense_set, power=power_set, exp=exp_set, death_function=monster_death)
                    ai_component = BasicMonster()               
                    monster = Object(x*TILE_WIDTH, y*TILE_HEIGHT, images[3], 'orc', blocks=True, fighter=fighter_component, ai=ai_component, image_num=3, fighter_list=[hp_set, defense_set, power_set, exp_set, 'monster_death'])
                elif choice == 'troll':
                    hp_set, defense_set, power_set, exp_set = 30, 2, 8, 100
                    fighter_component = Fighter(hp=hp_set, defense=defense_set, power=power_set, exp=exp_set, death_function=monster_death)
                    ai_component = BasicMonster()               
                    monster = Object(x*TILE_WIDTH, y*TILE_HEIGHT, images[4], 'troll', blocks=True, fighter=fighter_component, ai=ai_component, image_num=4, fighter_list=[hp_set, defense_set, power_set, exp_set, 'monster_death'], appended='objects')
                objects.append(monster)
                level_map[x][y].entity = monster # Places monster?
                monster.tile = level_map[x][y]
...
def play_game():
    ...
    global player_action, message_log, message_log_toggle, home_objects
...
def active_effects():
    ...
    if 'healing potion' in inventory_cache:
        ...
        friendly = False
    if 'scroll of fireball' in inventory_cache:
        dig = True
    else:
        dig = False
...
class Object:
    ...
    def __init__(self, x, y, image, name, blocks=False, fighter=None, ai=None, item=None, equipment=None, image_num=False, fighter_list=False, item_list=False, equipment_list=False, appended=False):
        ...
        # Allows objects to be saved
        parameters = [name, x, y, image_num, blocks, fighter_list, ai, item_list, equipment_list, appended]
        for i in range(len(parameters)):
            if (str(parameters[i])[0]+'0')[0] == '<':
                parameters[i] = 'SURFACE'
        save_objects.append(parameters)
...
class Fighter:
    ...
    def attack(self, target):
        ...
        if self.owner.name != target.name:
...
def save_game():
    ...
    #game_data['home_objects'] = home_objects
    #game_data['home_positions'] = home_object_positions
    #game_data['objects'] = objects
    #game_data['player'] = player
    #game_data['stairs'] = stairs
    game_data['objects'] = save_objects
...
def load_game():
    ...
    global game_msgs, game_msgs_data, game_state, player_action, message_log, save_objects
    with open('Evolution.dat', 'rb') as file:
        file_loader=pickle.Unpickler(file)
        game_data=file_loader.load()
    #print(f'objects: {objects}')
    #print(f"game_data: {game_data['objects']}")
    #print(f'save_objects: {save_objects}')
    #objects = []
    ...
    #save_objects = game_data['objects']
    #for i in range(len(save_objects)):
    #    # parameters = [name, x, y, image_num, blocks, fighter_list, ai, item_list, equipment_list]
    #    #print(save_objects[i])
    #    if save_objects[i][5]: # Fighter
    #        if save_objects[i][0] == 'player':
    #            ai_component = None
    #            fighter_component = Fighter(hp=save_objects[i][5][0], defense=save_objects[i][5][1], power=save_objects[i][5][2], exp=save_objects[i][5][3], death_function=player_death)
    #        else:
    #            ai_component = BasicMonster()
    #            fighter_component = Fighter(hp=save_objects[i][5][0], defense=save_objects[i][5][1], power=save_objects[i][5][2], exp=save_objects[i][5][3], death_function=monster_death)
    #    else:
    #        ai_component = None
    #        fighter_component = None
    #    if save_objects[i][7]: # Item
    #        if save_objects[i][0] == 'healing potion':
    #            item_component = Item(use_function=cast_heal)
    #        elif save_objects[i][0] == 'scroll of lightning bolt':
    #            item_component = Item(use_function=cast_lightning)
    #        elif save_objects[i][0] == 'scroll of fireball':
    #            item_component = Item(use_function=cast_fireball)
    #        elif save_objects[i][0] == 'scroll of confusion':
    #            item_component = Item(use_function=cast_confuse)
    #    else:
    #        item_component = None
    #    if save_objects[i][8]: # Equipment
    #        equipment_component = Equipment(slot=save_objects[i][8][0], power_bonus=save_objects[i][8][1])
    #    else:
    #        equipment_component = None
    #    name = save_objects[i][0]
    #    name = Object(save_objects[i][1], save_objects[i][2], images[int(save_objects[i][3])], save_objects[i][0], blocks=save_objects[i][4], fighter=fighter_component, ai=ai_component, item=item_component, equipment=equipment_component, image_num=save_objects[i][3], fighter_list=save_objects[i][5], item_list=save_objects[i][7], equipment_list=save_objects[i][8])
    #    if i == 0:
    #        player = name
    #        w = ROOM_MAX_SIZE # random.randint(ROOM_MIN_SIZE, ROOM_MAX_SIZE) # Sets a random width and height for a single room
    #        h = ROOM_MAX_SIZE # random.randint(ROOM_MIN_SIZE, ROOM_MAX_SIZE)
